...
function renderEmails(emailsContent:any) {
  const emailsContainer: Element | null =
    document.querySelector(".emails");

  if (emailsContainer) {
    // Now it's safe to access properties or methods on emailsContainer
    // For example:
    emailsContainer.textContent = "Some content";
  } else {
    console.error("emailsContainer is null");
  }
  ...
  if (emailsContent.length === 0) {
   ...
   emailsContent.sort((a: any, b: any) => {
     const dateA = new Date(a.date);
     const dateB = new Date(b.date);
     return dateB.getTime() - dateA.getTime(); // Explicitly convert Date objects to numbers
   });


    emailsContent.forEach((content:any) => {
...
function fetchDataAndUpdates() {
  ...
  emailsContent.forEach((email:any) => {
    const messageDate:any = new Date(email.date);
...
setInterval(fetchDataAndUpdates, 3000);
